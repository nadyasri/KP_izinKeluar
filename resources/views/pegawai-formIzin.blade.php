...
<form action="{{ route('pegawai-kirimIzin') }}" method="POST" class="bg-gray-100 p-6 rounded-lg shadow-md">
    @csrf
    <input type="hidden" name="groupId_pengirim" value="{{ $groupid_pengirim}}">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
            <label for="nama" class="block text-sm font-medium text-gray-700">Nama</label>
            <input type="text" name="nama" id="nama" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm" value="{{ $user->nama ?? ''}}" readonly>
        <div>
